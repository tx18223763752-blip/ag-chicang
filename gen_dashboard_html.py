...
DEEPSEEK_TIMEOUT_MS = 60000  # 常规 AI 分析接口超时兜底（毫秒）
DEEPSEEK_MP_TIMEOUT_MS = 180000  # 豆粕/棕榈油持仓价格分析超时（毫秒），prompt 数据量大时放宽
...
function syncContractOptions() {
  ...
  if (!list.includes(state.contract)) {
    const mc = pickMainContract(state.variety);
    state.contract = (mc && list.includes(mc.contract)) ? mc.contract : (list[0] || null);
  } else {
    cSel.value = state.contract;
  }
}
...
function seatTotalOI(variety, contract, seat) {
  ...
  if (!ser) return 0;
  const l = ser.long && ser.long.length ? (Number(ser.long[ser.long.length - 1]) || 0) : 0;
  const sh = ser.short && ser.short.length ? (Number(ser.short[ser.short.length - 1]) || 0) : 0;
  return l + sh;
}
...
function buildMPPrompt() {
  ...
  MP_VARIETIES.forEach(v => {
    ...
    const snapRow = snap.find(r => r.contract === mc.contract) || null;
    const dates = Array.isArray(ser.dates) ? ser.dates : [];
    const closes = Array.isArray(ser.close) ? ser.close : [];
    const n3 = Math.min(3, dates.length);
    ...
    L.push("最近收盘价：" + (snapRow && snapRow.close != null ? snapRow.close : "-") +
           ...
           "；最近3个交易日：" + (dates.length ? dates.slice(-n3).join(" / ") : "-") +
           "；3日收盘价序列：" + (closes.length ? closes.slice(-n3).join(" → ") : "-") + "。");
    if (!dates.length && !closes.length) {
      L.push(Object.keys(ser).length
        ? "（该品种暂无价格序列数据，以下仅基于席位持仓数据分析）"
        : "（该品种暂无可用持仓/价格数据，以下基于可得数据分析）");
    }
    L.push("各主要席位最近3日多头/空头净增减仓（手，正=增仓，负=减仓）：");
    SEATS.forEach(s => {
      const sd = (pool[mc.contract] && pool[mc.contract][s]) || null;
      const lc = sd && Array.isArray(sd.long_change) ? sd.long_change : [];
      const sc = sd && Array.isArray(sd.short_change) ? sd.short_change : [];
      if (!lc.length && !sc.length) { L.push("  " + s + "：无数据"); return; }
      const lnet = lc.slice(-n3).reduce((a, b) => a + (Number(b) || 0), 0);
      const snet = sc.slice(-n3).reduce((a, b) => a + (Number(b) || 0), 0);
      L.push("  " + s.replace("期货", "") + "：多头净增减 " + (lc.length ? fmtSigned(lnet) : "无") +
             "；空头净增减 " + (sc.length ? fmtSigned(snet) : "无"));
    });
    L.push("");
  });
  L.push("===== 输出要求 =====");
...
function renderMPReport(text) {
  const raw = String(text || "").trim();
  if (!raw) return '<div class="ai-empty">AI 未返回分析内容，请稍后重试。</div>';
  const clean = stripMarkdown(raw);
  const lines = clean.split("\\n").map(function (s) { return s.trim(); }).filter(function (s) { return s.length > 0; });
  ...
  lines.forEach(function (line) {
    ...
    if (m) {
      ...
      if (body) map[k].push(body);
      return;
    }
    if (line.indexOf("不构成任何投资建议") >= 0) { risk = line; return; }
    if (cur && map[cur]) { map[cur].push(line); return; }
    extras.push(line);
  });
  const tagCls = { "持仓变动": "q", "价格关系": "h", "结论": "c" };
  let html = "";
  MP_VARIETIES.forEach(function (v) {
    const groups = ["持仓变动", "价格关系", "结论"].map(function (t) {
      return { t: t, ls: (map[v + "|" + t] || []).filter(function (s) { return s.length > 0; }) };
    });
    const hasAny = groups.some(function (g) { return g.ls.length > 0; });
    if (!hasAny) return;
    const m = meta[v];
    ...
    groups.forEach(function (g) {
      ...
      html += '<div class="ai-card"><span class="ai-tag ai-tag-' + (tagCls[g.t] || "x") + '">' + g.t + "</span>";
      if (g.t === "结论") {
        html += '<div class="ai-text">' + g.ls.map(function (l) { return escapeHtml(l); }).join("<br>") + "</div>";
      } else {
        html += '<ul class="mp-list">' + g.ls.map(function (l) { return "<li>" + escapeHtml(l) + "</li>"; }).join("") + "</ul>";
      }
      html += "</div>";
    });
    html += "</div></div>";
  ...
  if (!html) {
    const fallback = [].concat(extras, ["持仓变动", "价格关系", "结论"].reduce(function (a, t) {
      return a.concat(map["豆粕|" + t] || [], map["棕榈油|" + t] || []);
    }, [])).filter(function (s) { return s && s.length > 0; }).map(function (s) { return escapeHtml(s); }).join("<br>");
    let msg = '<div class="ai-empty">AI 未返回有效的结构化要点，未识别到【豆粕/棕榈油·持仓变动/价格关系/结论】标签内容，请稍后重试。</div>';
    if (fallback) msg += '<div class="ai-body">' + fallback + "</div>";
    if (risk) msg += '<div class="mp-risk">' + escapeHtml(risk) + "</div>";
    return msg;
  }
  if (extras.length) html += '<div class="ai-para">' + extras.map(function (s) { return escapeHtml(s); }).join("<br>") + "</div>";
...
async function runMPAnalysis() {
  ...
  btn.disabled = true;
  btn.textContent = "分析中…";
  const mpTimeout = DEEPSEEK_CONFIG.mp_timeout_ms || DEEPSEEK_CONFIG.timeout_ms;
  out.innerHTML = '<div class="ai-loading">正在生成豆粕/棕榈油持仓-价格分析，数据较多，约需 1~2 分钟，请稍候…</div>';
  const ctrl = new AbortController();
  const timer = setTimeout(function () { ctrl.abort(); }, mpTimeout);
  try {
...
def main():
    ...
    html = (HTML_TEMPLATE
            ...
            .replace("__DEEPSEEK_CONFIG__", json.dumps({
                ...
                "mp_timeout_ms": DEEPSEEK_MP_TIMEOUT_MS,
            }, ensure_ascii=False)))
